...
class site_api:
    ...
    def listApi(self):
        ...
        for i in range(len(_list)):
            ...
            _list[i]['path'] = _list[i]['path']

        _ret = {}
        _ret['data'] = _list
        _ret['url'] = os.environ["KODBOX_URL"]
    ...
    def getPhpVersionApi(self):
        data = [{'version':'00',"name":'纯静态'},{'version':'74',"name":'PHP-7.4'}]
        return mw.getJson(data)
...
# PROXY-END/
    ...
    def fcNginxAddConf(self):
        app_name = "nginx-web"
        if self.phpVersion == "74":
            app_name = "php74-web"
        source_tpl = mw.getRunDir() + f'/fc-template/{app_name}/s.yaml'
        vhost_file = self.sitePath + '/' + 's.yaml'
